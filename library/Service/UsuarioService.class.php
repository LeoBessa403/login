...
class  UsuarioService extends AbstractService
{
     ...
     */
    public function salvaUsuarioInicial($coPessoa, $dadosEmail)
    {
    ...
    public function getUsuariosCoordenadoresCombo()
    {
        $comboUsuarios = [
            '' => Mensagens::MSG_SEM_ITEM_SELECIONADO
        ];
        $usuarios = $this->ObjetoModel->getUsuariosCoordenadores();
        /** @var UsuarioEntidade $usuario */
        foreach ($usuarios as $usuario) {
            $comboUsuarios[$usuario->getCoUsuario()]
                = Valida::Resumi(strtoupper($usuario->getCoPessoa()->getNoPessoa()), 25);
